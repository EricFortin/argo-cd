...
const (
	...
	// LabelOrphanedApplicationSet is a label that is added when an Application is no longer generated by an ApplicationSet and can't be deleted because of its applicationsSync policy(e.g. create-update)
	// A label is used for searching purposes
	LabelOrphanedApplicationSet = "applicationset.argocd.argoproj.io/orphaned"

	// AnnotationOrphanedByApplicationSet is an annotation that is added when an Application is no longer generated by an ApplicationSet and can't be deleted because of its applicationsSync policy(e.g. create-update)
	// Annotation value format is <applicationset-ns>/<applicationset-name>
	AnnotationOrphanedByApplicationSet = "applicationset.argocd.argoproj.io/orphaned-by"
)
