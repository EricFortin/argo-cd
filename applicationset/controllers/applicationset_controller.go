...
func (r *ApplicationSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if utils.DefaultPolicy(applicationSetInfo.Spec.SyncPolicy, r.Policy, r.EnablePolicyOverride).AllowDelete() {
		// Delete applications that are no longer generated by this applicationset.
		...
		// Orphan application since we can't delete them.
		err = r.orphanApplications(ctx, logCtx, applicationSetInfo, generatedApplications)
		if err != nil {
			_ = r.setApplicationSetStatusCondition(ctx,
				&applicationSetInfo,
				argov1alpha1.ApplicationSetCondition{
...
func (r *ApplicationSetReconciler) orphanApplications(ctx context.Context, logCtx *log.Entry, applicationSet argov1alpha1.ApplicationSet, desiredApplications []argov1alpha1.Application) error {
	// Save current applications to be able to orphan the ones that are not in appList
	current, err := r.getCurrentApplications(ctx, applicationSet)
	if err != nil {
		return fmt.Errorf("error getting current applications: %w", err)
	}

	m := make(map[string]bool) // Will holds the app in appList for the orphaning process

	for _, app := range desiredApplications {
		m[app.Name] = true
	}

	// Label apps that are not in m[string]bool
	var firstError error
	for _, app := range current {
		logCtx = logCtx.WithFields(applog.GetAppLogFields(&app))
		_, exists := m[app.Name]
		...
		if !exists {
			...
			app.Labels[common.LabelOrphanedApplicationSet] = "true"

			if app.Annotations == nil {
				app.Annotations = map[string]string{}
			}
			app.Annotations[common.AnnotationOrphanedByApplicationSet] = fmt.Sprintf("%s/%s", applicationSet.Namespace, applicationSet.Name)

			err := r.Update(ctx, &app)
			if err != nil {
				return fmt.Errorf("error orphaning application: %w", err)
			}

			r.Recorder.Eventf(&applicationSet, corev1.EventTypeNormal, "Orphaned", "Orphaned Application %q", app.Name)
			logCtx.Log(log.InfoLevel, "Orphaned application")
		}
	}
	return firstError
}
