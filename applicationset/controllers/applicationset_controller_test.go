...
func TestCreateOrUpdateInCluster(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{
			name: "Ensure that labels and annotations are added (via update) into an existing application",
			appSet: v1alpha1.ApplicationSet{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "name",
					Namespace: "namespace",
...
func TestOrphanApplications(t *testing.T) {
	scheme := runtime.NewScheme()
	err := v1alpha1.AddToScheme(scheme)
	require.NoError(t, err)

	for _, c := range []struct {
		// appSet is the application set on which the orphan function is called
		appSet v1alpha1.ApplicationSet
		// existingApps is the current state of Applications on the cluster
		existingApps []v1alpha1.Application
		// desireApps is the apps generated by the generator that we wish to keep
		desiredApps []v1alpha1.Application
		// expected is the list of applications that we expect to exist after calling orphan
		expected []v1alpha1.Application
		// orphaned is the list of applications that we expect to have been orphaned after calling orphan
		orphaned []v1alpha1.Application
	}{
		{
			appSet: v1alpha1.ApplicationSet{
				ObjectMeta: metav1.ObjectMeta{
		...
		err = r.orphanApplications(t.Context(), log.NewEntry(log.StandardLogger()), c.appSet, c.desiredApps)
		require.NoError(t, err)
		...
		// Verify each of the orphaned objs have the label and annotation set
		for _, obj := range c.orphaned {
			got := &v1alpha1.Application{}
			err := client.Get(t.Context(), crtclient.ObjectKey{
				Namespace: obj.Namespace,
				Name:      obj.Name,
			...
			assert.Equal(t, got.Labels[argocommon.LabelOrphanedApplicationSet], "true")
			assert.Equal(t, got.Annotations[argocommon.AnnotationOrphanedByApplicationSet], fmt.Sprintf("%s/%s", c.appSet.Namespace, c.appSet.Name))
		}
	}
}
